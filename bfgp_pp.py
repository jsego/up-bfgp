...
class BestFirstGeneralizedPlannerPP(engines.Engine, engines.mixins.OneshotPlannerMixin):
    ...
    def __init__(self, **options):
        ...
        assert 'theory' in options
        self._theory = options['theory']
        assert self._mode != 'synthesis' or 'program_lines' in options
        self._program_lines = options['program_lines']
        assert 'problem_folder' in options
        self._problem_folder = options['problem_folder']
        assert 'evaluation_functions' in options  # this shouldn't be mandatory
        self._evaluation_functions = options['evaluation_functions']
    ...
    @staticmethod
    def supported_kind():
        # ToDo: update this info to FD style
            ...
            while True:
                ...
                if random.random() < self.restart_probability:
                    ...
                    if res:
                        ...
                        return up.engines.PlanGenerationResult(PlanGenerationResultStatus.SOLVED_SATISFICING, resplan,
                                                               self.name)
                    else:
